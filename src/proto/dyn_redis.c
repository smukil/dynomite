...
rstatus_t create_write_script(struct context *ctx, struct conn *conn,
    uint64_t timestamp, uint32_t keylen, uint8_t *key, struct msg *orig_msg,
    msg_type_t cmd_type, bool is_repair, struct msg **result_msg_ptr) {
  ...
  */
  ...
  loga("Rewriting %s query. TIMESTAMP: %u", op_str, timestamp);
  // Get a new 'msg' structure.
  struct msg *new_msg = msg_get(conn, true, __FUNCTION__);
  if (new_msg == NULL) {
    ret_status = DN_ENOMEM;
    goto error;
  }
  ...
  rstatus_t prepend_status = msg_prepend_format(
      new_msg, REPAIR_WRITE_SCRIPT_FMT_STRING, keylen, key, add_set_key,
...
rstatus_t redis_make_repair_query(struct context *ctx, struct conn *conn,
  ...
  switch(msg_type) {
    case MSG_REQ_REDIS_SET:
      ...
      ret_status = create_write_script(ctx, conn, timestamp, keylen, key_copy, orig_msg,
          msg_type, true, new_msg_ptr);
...
rstatus_t redis_rewrite_query_with_timestamp_md(struct msg *orig_msg, struct context *ctx,
  ...
  *did_rewrite = false;
  ...
  if (read_repairs != 1) return DN_OK;
  ...
  if (read_repairs == 1 && orig_msg->is_read == 0) {
    ...
    switch(orig_msg->type) {
      case MSG_REQ_REDIS_APPEND:
        ret_status = create_write_script(ctx, orig_msg->owner, orig_msg->timestamp,
            req_key_len, req_key, orig_msg, orig_msg->type, false, new_msg_ptr);
        if (ret_status != DN_OK) goto error;
        *did_rewrite = true;
        return ret_status;
      case MSG_REQ_REDIS_SET:
        ...
        switch (ch) {
          case LF:
            ...
            if (redis_arg0(r)) {
              ...
              if (r->rntokens != 1 && (read_repairs == 1 && r->rntokens != 2)) {
                goto error;
              }
              state = SW_ARG1_LEN;
